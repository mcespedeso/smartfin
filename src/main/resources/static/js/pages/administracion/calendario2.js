...
$(document).ready(function(){
    ...
    $('#ms_checkEstado').on('ifUnchecked', function(event){
      ...
      $('#ms_checkPublic').on('ifUnchecked', function(event){
      
      	  publico = false;
      	$("#labelPublico").text("  PRIVADO (Visible solo para tu organizacion)")
      	});
        $('#ms_checkPublic').on('ifChecked', function(event){
        	publico = true;
        	$("#labelPublico").text("  PUBLICO (Visible para todas las organizaciones)")
        });
...
function guardar(ban){
	if(validate() == 0){
		...
		var pojo = pojoCalendario($("#idcalendario").val(), 0, $("#descripcion").val() ,
				$("#fechadesde").val(), $("#fechahasta").val(), estado, publico)
