...
function getUsuarios(){
	...
	$.ajax({
		type : "GET",
		url : context + "/private/socioeconomica/all",		
...
function insertForm(fila) {
	...
	document.getElementById('ms_idsocioeconomica').value = fila.cells[0].innerHTML;
	document.getElementById('ms_descripcion').value = fila.cells[1].innerHTML;
	document.getElementById('ms_formula').value =fila.cells[2].innerHTML ;
...
function clearForm() {
	...
	table.$('tr.selected').removeClass('selected');
	document.getElementById('ms_idsocioeconomica').value = 0;
	document.getElementById('ms_descripcion').value = "";
	document.getElementById('ms_formula').value = "";
...
function guardar(){
	...
	if(validate() == 0){	
		var path = context + "/private/socioeconomica"
		var semaforo =pojoSocioeconomica(document.getElementById('ms_idsocioeconomica').value, document.getElementById('ms_descripcion').value,document.getElementById('ms_formula').value,  estado)
				console.log(semaforo)
		if(document.getElementById('ms_idsocioeconomica').value == 0){
			//insert
			$.ajax({
				type : "POST",
				url : path+"/insert",
				data :{
					"socioeconomica" : semaforo
				},
				success : function(data) {
					console.log(data)
					data = JSON.stringify(data);
