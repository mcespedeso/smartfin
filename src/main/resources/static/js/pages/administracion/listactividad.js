...
function getAll(){
	...
	$.ajax({
		type : "GET",
		url : context + "/private/actividad/all",		
		success : function(data) {
			...
			if (jsonobject.estado == 'ERROR') {
				...
				$.each(jsonobject.datos, function(index, element) {
					...
					var estado = "";
					var accion = "";
					if(element.estados.idestado == 6){
						estado = '<span class="label label-primary pull-center">Aprobado</span>'
						accion = '<div class="btn-group"><a href="detalleactividad?'+element.idactividad+'"><button class="btn-white btn btn-xs" >Ver</button><a/><button class="btn-danger btn btn-xs" onclick="eliminar('+element.idactividad+')">Eliminar</button></div>';
					}else if(element.estados.idestado == 5){
						estado ='<span class="label label-success pull-center">Reportado</span>'
						accion = '<div class="btn-group"><a href="detalleactividad?'+element.idactividad+'"><button class="btn-white btn btn-xs" >Ver</button><a/><a href="aprobaractividad?'+element.idactividad+
						'"><button class="btn-primary btn btn-xs">Aprobar</button></a><button class="btn-danger btn btn-xs" onclick="eliminar('+element.idactividad+')">Eliminar</button></div>'
					}else if(element.estados.idestado == 3){
						estado ='<span class="label label-danger pull-center">Cancelado</span>'
							accion = '<div class="btn-group"><a href="detalleactividad?'+element.idactividad+'"><button class="btn-white btn btn-xs" >Ver</button><a/><a href="aprobaractividad?'+element.idactividad+
							'"><button class="btn-primary btn btn-xs">Aprobar</button></a><button class="btn-danger btn btn-xs" onclick="eliminar('+element.idactividad+')">Eliminar</button></div>'
					}
					...
					table.rows.add( [ {
							"0": element.idactividad,
							"1": element.pregunta.titulo,
							"2": element.descripcion,
							"3": element.empresa.descripcion,
							"4": estado,
							"5": accion
...
function eliminar(id){
	$.ajax({
		type : "POST",
		url : context + "/private/actividad/delete/"+id,
		...
		success : function(data) {
			...
			if (jsonobject.estado == "ERROR") {
				...
				alertify .alert("Proceso Correcto", jsonobject.datos , function(){ 		
				});
